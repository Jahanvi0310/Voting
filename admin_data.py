...
class admin_details:
    def Search(self,event):
        ...
        cor=con.cursor()
        cor.execute("select f_name, l_name,email, aadhar from registration where aadhar=%s",(self.adhaar.get(),))
        rec=cor.fetchall()
        if len(rec):
            p=1
            for row in rec:
    ...
    def __init__(self):
        ...
        con=c.connect(host="localhost",user="root",password="root@123",database="voting")
        cor=con.cursor()
        cor.execute("select f_name, l_name, aadhar ,dob , gender , party from registration")  
        rec=cor.fetchall()
        con.commit()
        ...
        tab2aid=t.Entry(self.tab2,textvariable=self.adhaar)
        ...
        #tab2search.bind(tab2,"<Button-1>",lambda event:self.search(event))
